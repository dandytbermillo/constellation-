...
import { logConstellationFocus, logDepthCalculation, logDoubleClick, debugLog } from '@/utils/debugLog';
...
export function useConstellation() {
  ...
  const branchDepthOverrides = useMemo(() => {
    ...
    debugLog({
      component: 'SpotlightCascade',
      action: 'depth:recompute',
      metadata: {
        activeSpotlight: state.activeSpotlight,
        pinnedSpotlights: state.pinnedSpotlights,
        inlineExpanded: Array.from(state.inlineExpandedConstellations),
        expandedConstellations: Array.from(state.expandedConstellations),
        knowledgeBaseId: state.knowledgeBaseId,
        spotlightBranches: spotlightBranches.map(branch => ({
    ...
    const assignItem = (item: ConstellationItem | undefined, layer: number) => {
      ...
      const normalizedId = item ? normalizeId(item.id) : null;
      if (item && (state.inlineExpandedConstellations.has(normalizedId) || spotlightBranchIdSet.has(normalizedId))) {
        debugLog({
          component: 'SpotlightCascade',
          action: 'depth:assignItem',
          metadata: {
            itemId: item.id,
            normalizedId,
            title: item.title,
            layer,
            inlineExpanded: state.inlineExpandedConstellations.has(normalizedId),
            spotlightBranch: spotlightBranchIdSet.has(normalizedId),
            isCenter: item.isCenter,
    ...
    const visitDescendants = (
      ...
      childCandidates.forEach(child => {
        ...
        if (shouldVisit) {
          if (inlineExpanded || spotlightExpanded) {
            debugLog({
              component: 'SpotlightCascade',
              action: 'visit:traversing',
              metadata: {
                parentId: item.id,
                childId: child.id,
                childTitle: child.title,
    ...
    const processBranch = (branchId: string, offset: number, options?: { inline?: boolean }) => {
      ...
      debugLog({
        component: 'SpotlightCascade',
        action: 'traversal:processBranch',
        metadata: {
          branchId,
          offset,
          modeHint: isInlineMode ? 'inline' : (isKnowledgeBaseBranch ? 'knowledge-base' : 'spotlight'),
          activeSpotlightId,
          pinned: Array.from(pinnedNormalizedIds)
        }
      });
      ...
      debugLog({
        component: 'SpotlightCascade',
        action: 'traversal:branchMode',
        metadata: {
          branchId,
          branchMode,
          rootLayer,
          branchHasInline: state.inlineExpandedConstellations.has(branchId),
          rootTitle: branchRootItem?.title
        }
      });
      ...
      const shouldTraverseRoot =
        branchMode !== 'knowledge-base'
        || (branchMode === 'knowledge-base' && activeSpotlightId !== state.knowledgeBaseId);

      if (rootExpanded && shouldTraverseRoot) {
        debugLog({
          component: 'SpotlightCascade',
          action: 'traversal:rootVisit',
          metadata: {
            branchId,
            branchMode,
            rootLayer,
            branchTitle: branchRootItem.title,
            branchHasInline: state.inlineExpandedConstellations.has(branchId)
          }
        });
  ...
  const promoteBranchToSpotlight = useCallback((folderId: string) => {
    ...
    setState(prevState => {
      ...
      debugLog({
        component: 'SpotlightCascade',
        action: 'promote:before',
        metadata: {
          rootId: folderId,
          normalizedRoot,
          inlineExpanded: Array.from(prevState.inlineExpandedConstellations),
          expanded: Array.from(prevState.expandedConstellations),
      ...
      const nextInline = new Set(prevState.inlineExpandedConstellations);
      const snapshotInlineNormalized = new Set(branchSnapshot.inline.map(id => normalizeId(id)));
      const removedInlineSet = new Set<string>();

      const removeInlineVariants = (id: string | null | undefined) => {
        if (!id) return;
        const normalizedId = normalizeId(id);
        if (nextInline.delete(id)) removedInlineSet.add(normalizedId);
        if (nextInline.delete(normalizedId)) removedInlineSet.add(normalizedId);
        const centerId = `${normalizedId}_center`;
        if (nextInline.delete(centerId)) removedInlineSet.add(normalizedId);
      };

      branchSnapshot.inline.forEach(id => {
        removeInlineVariants(id);
      });

      const resolveItem = (id: string | null | undefined): ConstellationItem | undefined => {
        if (!id) return undefined;
        const normalizedId = normalizeId(id);
        return itemsByNormalizedId.get(normalizedId) || itemsById.get(id);
      };

      let ancestorItem = resolveItem(folderId);
      while (ancestorItem && ancestorItem.parentId) {
        const ancestorId = ancestorItem.parentId;
        const normalizedAncestor = normalizeId(ancestorId);
        if (
          nextInline.has(ancestorId) ||
          nextInline.has(normalizedAncestor) ||
          nextInline.has(`${normalizedAncestor}_center`)
        ) {
          removeInlineVariants(ancestorId);
        }

        if (knowledgeBaseNormalized && normalizedAncestor === knowledgeBaseNormalized) {
          break;
        }

        ancestorItem = resolveItem(ancestorId);
      }

      if (knowledgeBaseNormalized) {
        removeInlineVariants(knowledgeBaseNormalized);
        removedInlineSet.add(knowledgeBaseNormalized);
        removedInlineSet.add(`${knowledgeBaseNormalized}_center`);
      }
      ...
      const removedInline = Array.from(removedInlineSet);
      const clearedInlineAncestors = removedInline.filter(id => !snapshotInlineNormalized.has(id));
      debugLog({
        component: 'SpotlightCascade',
        action: 'promote:state',
        metadata: {
          rootId: folderId,
          normalizedRoot,
          nextInline: Array.from(nextInline),
          nextExpanded: Array.from(nextExpanded),
          nextActive,
          nextPinned,
          removedInline,
          clearedAncestors: clearedInlineAncestors
        }
      });
  ...
  const demoteBranchFromSpotlight = useCallback((folderId: string) => {
    ...
    setState(prevState => {
      ...
      if (snapshot.clearedInlineAncestors) {
        snapshot.clearedInlineAncestors.forEach(id => {
          const normalizedId = normalizeId(id);
          nextInline.add(normalizedId);
        });
      }
      ...
      debugLog({
        ...
        metadata: {
          ...
          nextActive,
          nextPinned,
          restoredInlineAncestors: snapshot.clearedInlineAncestors ?? []
        }
      });
